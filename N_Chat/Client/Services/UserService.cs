...
namespace N_Chat.Client.Services{
    public class UserService : IUserService{
        ...
        public async Task<UserModel> GetUserByName(string userName){
            try{
                /*This will call the PostAsync method with the interpolated string as the first parameter, a null HttpContent object as the second parameter, and the CancellationToken.None as the third parameter.*/
                Console.WriteLine("GetUserByName(string userName) just ran!!!!");
                var result = await httpClient.GetFromJsonAsync<UserModel>($"api/user/getbyname/{userName}");
                
                Console.WriteLine("row 31: result = " + result);
                return result;
            }
            catch (Exception e){
                Console.WriteLine("'Error!!!!!!! UserService.cs:32. Error message: ");
    ...
    public interface IUserService{
        ...
        Task<UserModel> GetUserByName(string userName); // Get user by users userName
